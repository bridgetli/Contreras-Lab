...
input_file_name = 'sRNAs_coordinates_and_seeds_final.xlsx'
file = pd.ExcelFile(input_file_name)

# Load workbook so you can add new sheets w/o overwriting existing ones
# Use the specific name of your Excel file
book = load_workbook(input_file_name)
writer = pd.ExcelWriter(input_file_name, engine='openpyxl') 
writer.book = book
...
seed_regions_df = file.parse('seed_regions')
...
# Save sheet.
seed_regions_df.to_excel(writer, 'seed_regions', index=False)
writer.save()
